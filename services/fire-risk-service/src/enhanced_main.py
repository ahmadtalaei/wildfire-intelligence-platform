    ...
    try:
        ...
        if risk_score > 0.8:
            recommendations.extend([
                ...
                "[HELICOPTER] Deploy aerial suppression resources",
                "📢 Issue evacuation warnings for high-risk areas",
                "⛔ Impose strict fire activity restrictions"
            ])
        elif risk_score > 0.6:
            recommendations.extend([
                "[WARNING] HIGH: Increase fire weather monitoring",
                "[FIRE] Restrict outdoor burning activities", 
                "👨‍🚒 Alert fire suppression crews",
                "[SATELLITE_ANTENNA] Enhance detection systems"
            ])
        elif risk_score > 0.3:
            recommendations.extend([
                "[LIGHTNING] MODERATE: Monitor weather conditions",
                "[MAGNIFYING_GLASS] Increase patrol frequency",
                "[CLIPBOARD] Review suppression readiness"
            ])
        else:
            recommendations.extend([
                "[CHECK] LOW: Standard fire prevention measures",
                "[BAR_CHART] Continue routine monitoring"
            ])
        
        # Weather-specific recommendations
        if request.weather.humidity < 15:
            recommendations.append("[DROPLET] Critical humidity levels - maximize water resources")
        ...
        if request.fuel_moisture < 10:
            recommendations.append("[SEEDLING] Critically dry fuels - implement fuel breaks")
