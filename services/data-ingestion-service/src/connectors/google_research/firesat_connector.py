...
try:
    from ...config.geographic_bounds import CALIFORNIA_BOUNDS
except ImportError:
    try:
        ...
        CALIFORNIA_BOUNDS = {
            'lat_min': settings.CA_lat_min,
            'lat_max': settings.CA_lat_max,
            'lon_min': settings.CA_lon_min,
            'lon_max': settings.CA_lon_max
        }
    ...
    async def _poll_earth_engine_data(
        ...
        try:
            ...
            if geographic_bounds:
                ...
                # California bounds using centralized config
                region = ee.Geometry.Rectangle([
                    CALIFORNIA_BOUNDS['lon_min'], CALIFORNIA_BOUNDS['lat_min'],
                    CALIFORNIA_BOUNDS['lon_max'], CALIFORNIA_BOUNDS['lat_max']
                ])
    ...
    async def _poll_simulated_data(
        ...
        for i in range(num_detections):
            ...
            if geographic_bounds:
                ...
                # Use centralized California bounds for random coordinate generation
                lat = np.random.uniform(CALIFORNIA_BOUNDS['lat_min'], CALIFORNIA_BOUNDS['lat_max'])
                lon = np.random.uniform(CALIFORNIA_BOUNDS['lon_min'], CALIFORNIA_BOUNDS['lon_max'])
