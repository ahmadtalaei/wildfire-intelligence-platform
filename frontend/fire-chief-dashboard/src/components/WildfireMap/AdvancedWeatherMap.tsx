...
const AdvancedWeatherMap: React.FC<AdvancedWeatherMapProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const baseLayers = {
        '[SATELLITE] Satellite (ESRI)': window.L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
          maxZoom: 19,
          attribution: 'Imagery (C) Esri, DigitalGlobe, GeoEye, Earthstar Geographics, USDA FSA'
        }),
        '[MAP] Topographic (USGS)': window.L.tileLayer('https://basemap.nationalmap.gov/arcgis/rest/services/USGSTopo/MapServer/tile/{z}/{y}/{x}', {
          maxZoom: 16,
        ...
        '[BAR_CHART] NAIP Imagery': window.L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/USDA/NAIP/MapServer/tile/{z}/{y}/{x}', {
          maxZoom: 17,
          attribution: 'NAIP (C) USDA FSA'
        })
      };

      baseLayers['[SATELLITE] Satellite (ESRI)'].addTo(map);

      // Professional weather and operational overlays
      const weatherLayers = {
        '[RAIN_CLOUD] Precipitation (NOAA)': window.L.tileLayer('https://mesonet.agron.iastate.edu/cache/tile.py/1.0.0/nexrad-n0q-900913/{z}/{x}/{y}.png', {
          opacity: 0.7,
          attribution: 'NEXRAD (C) NOAA'
        }),
        '[THERMOMETER] Temperature': window.L.tileLayer('https://tile.openweathermap.org/map/temp_new/{z}/{x}/{y}.png', {
          opacity: 0.6,
          attribution: 'Temperature (C) OpenWeatherMap'
        }),
        '💨 Wind Speed': window.L.tileLayer('https://tile.openweathermap.org/map/wind_new/{z}/{x}/{y}.png', {
          opacity: 0.6,
          attribution: 'Wind (C) OpenWeatherMap'
        }),
        '[CLOUD] Cloud Cover': window.L.tileLayer('https://tile.openweathermap.org/map/clouds_new/{z}/{x}/{y}.png', {
          opacity: 0.5,
          attribution: 'Clouds (C) OpenWeatherMap'
        })
      };

      // Professional operational overlays
      const operationalLayers = {
        '[HELICOPTER] Helicopter Landing Zones': window.L.layerGroup(),
        '🏥 Emergency Services': window.L.layerGroup(),
        '🚰 Water Sources': window.L.layerGroup(),
        '[SATELLITE_ANTENNA] Communication Towers': window.L.layerGroup(),
        '🛣️ Evacuation Routes': window.L.layerGroup()
      };

      // Fire hazard and risk layers
      const fireHazardLayers = {
        '[FIRE] Fire Hazard Severity Zones': window.L.layerGroup(),
        '[HOUSE] Wildland Urban Interface': window.L.layerGroup(),
        '[EVERGREEN_TREE] Fuel Load Analysis': window.L.layerGroup()
      };
        ...
        veryHighRiskAreas.forEach((coords, index) => {
          const polygon = window.L.polygon(coords, {
            ...
            <div style="font-family: Arial, sans-serif; min-width: 250px;">
              <h4 style="color: #DC143C; margin: 0 0 8px 0;">[FIRE] VERY HIGH Fire Hazard Severity Zone</h4>
          ...
          `);
          fireHazardLayers['[FIRE] Fire Hazard Severity Zones'].addLayer(polygon);
        });
      ...
      setTimeout(() => {
          ...
          const marker = window.L.marker([helipad.lat, helipad.lng], { icon })
            .bindPopup(`
              <div style="font-family: Arial, sans-serif;">
                <h4 style="margin: 0 0 8px 0; color: #2E7D32;">[HELICOPTER] ${helipad.name}</h4>
            ...
            `);
          operationalLayers['[HELICOPTER] Helicopter Landing Zones'].addLayer(marker);
        });
        ...
        waterSources.forEach(water => {
          ...
          const marker = window.L.marker([water.lat, water.lng], { icon })
            .bindPopup(`
              <div style="font-family: Arial, sans-serif;">
                <h4 style="margin: 0 0 8px 0; color: #1565C0;">[DROPLET] ${water.name}</h4>
      ...
      legend.onAdd = function() {
        ...
        div.innerHTML = `
          <div style="font-weight: bold; margin-bottom: 10px; color: #d84315; font-size: 13px;">
            [FIRE] Fire Hazard Severity Zones (FHSZ)
          </div>
      ...
      map.on('mousemove', function(e: any) {
        ...
        if (coordsDisplay) {
          coordsDisplay.innerHTML = `Lat: ${lat}deg, Lng: ${lng}deg<br/>Zoom: ${map.getZoom()}`;
        }
      });
  ...
  useEffect(() => {
    ...
    const fetchWeatherData = async () => {
      ...
      try {
        ...
        const dataPromise = advancedWeatherService.fetchCaliforniaWeatherGrid(0.5); // 0.5deg ~= 55km resolution
        ...
        setTimeout(async () => {
          try {
            ...
            const enhancedData = await advancedWeatherService.fetchCaliforniaWeatherGrid(0.25); // 0.25deg ~= 25km resolution
  ...
  const createTemperatureHeatmap = useCallback((map: any) => {
    ...
    temperature.points.forEach((row, i) => {
      row.forEach((point, j) => {
        ...
        const normalizedTemp = Math.max(0, Math.min(1, (point.temperature - (-10)) / 50)); // -10degC to 40degC
        ...
        const circle = window.L.circle([lat, lng], {
          ...
          weight: 0,
          opacity: 0.6
        }).bindTooltip(`${point.temperature.toFixed(1)}degC`);
  ...
  const createWindVectors = useCallback((map: any) => {
    ...
    wind.points.forEach((row, i) => {
      ...
      row.forEach((point, j) => {
        ...
        }).bindTooltip(`${windSpeed.toFixed(1)} m/s @ ${point.direction.toFixed(0)}deg`);
      ...
      const marker = window.L.marker([detection.latitude, detection.longitude], {
        ...
        <div style="min-width: 250px; font-family: Arial, sans-serif;">
          <h3 style="margin: 0 0 10px 0; color: ${color}; display: flex; align-items: center;">
            [SATELLITE] Satellite Fire Detection
          </h3>
  ...
  const calculateFireWeatherIndex = useCallback(() => {
    ...
    if (avgTemp > 38) score += 3;      // > 100degF
    else if (avgTemp > 32) score += 2; // > 90degF
    else if (avgTemp > 27) score += 1; // > 80degF
