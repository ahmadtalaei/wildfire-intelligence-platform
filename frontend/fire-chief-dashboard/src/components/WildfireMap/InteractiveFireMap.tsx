...
const InteractiveFireMap: React.FC<InteractiveFireMapProps> = ({
  ...
  const calculateFireSpreadPredictions = () => {
    ...
    activeFires.forEach(fire => {
      ...
      const temperature = 92; // degF - from weather data
  ...
  const generateWindVectors = () => {
    ...
    for (let lat = mapCenter[0] - 0.5; lat <= mapCenter[0] + 0.5; lat += step) {
      for (let lng = mapCenter[1] - 0.5; lng <= mapCenter[1] + 0.5; lng += step) {
        ...
        const windDirection = 45 + (Math.random() - 0.5) * 30; // ~45deg +/- 15deg
        ...
        <Paper sx={{ position: 'absolute', top: 16, left: 16, p: 2, maxWidth: 300 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <Typography variant="body2">
              [LOCATION_PIN] Active Fires: {activeFires.length}
            </Typography>
            {timeHours > 0 && (
              <Typography variant="body2">
            ...
            <Typography variant="body2">
              [THERMOMETER] Conditions: 92degF, 8% RH (Red Flag Warning)
            </Typography>
          </Box>
        </Paper>
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Air color="primary" />
                        <Typography variant="body2">
                          Wind: 25 mph @ 45deg NE
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Thermostat color="error" />
                        <Typography variant="body2">
                          Temp: 92degF, Humidity: 8%
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Speed color="warning" />
