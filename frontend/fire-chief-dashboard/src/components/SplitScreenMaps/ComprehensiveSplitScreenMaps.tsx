...
const ComprehensiveSplitScreenMaps: React.FC<ComprehensiveSplitScreenMapsProps> = ({ lastRefresh }) => {
  ...
  const fetchNationwideFireData = async () => {
    ...
    try {
      console.log('[SATELLITE] Fetching comprehensive fire data from NASA FIRMS...');
  ...
  const initializeComprehensiveFireMap = () => {
    ...
    try {
      ...
      console.log('[MAP] Comprehensive fire map initialized');
  ...
  const addAllOperationalOverlays = (map: any) => {
    ...
    console.log('[HELICOPTER] Adding ALL operational overlays to map');
  ...
  const addFireHazardZones = (map: any) => {
    ...
    hazardZones.forEach(zone => {
      ...
      window.L.rectangle(zone.bounds, {
        ...
        <strong>[FIRE] ${zone.name}</strong><br>
  ...
  const addWildlandUrbanInterface = (map: any) => {
    ...
    wuiZones.forEach(zone => {
      ...
      window.L.rectangle(zone.bounds, {
        ...
        <strong>[HOUSE] ${zone.name}</strong><br>
    ...
    operationalData.forEach(overlay => {
      ...
      switch (overlay.type) {
        case 'helicopter_landing':
          iconHtml = '[HELICOPTER]'; color = overlay.status === 'threatened' ? '#FF9800' : '#4CAF50'; break;
        case 'water_source':
          iconHtml = '[DROPLET]'; color = '#2196F3'; break;
        case 'emergency_service':
          iconHtml = overlay.name.includes('Fire') ? '🚒' : overlay.name.includes('Police') ? '🚔' : '🚑';
          color = '#F44336'; break;
        case 'communication_tower':
          iconHtml = '[SATELLITE_ANTENNA]'; color = '#9C27B0'; break;
        case 'weather_station':
          iconHtml = '[THERMOMETER]'; color = '#FF9800'; break;
        case 'fuel_analysis':
          iconHtml = '[HERB]'; color = '#4CAF50'; break;
        default:
          iconHtml = '[LOCATION_PIN]';
      }
        ...
        .bindPopup(`
          <div style="font-family: Arial; min-width: 300px;">
            ...
            <div style="background: rgba(255, 69, 0, 0.1); padding: 10px; border-radius: 6px; margin-top: 10px;">
              <strong style="color: #FF4500;">[WARNING] LIVE SATELLITE DATA</strong><br>
  ...
  const generateWindyURL = () => {
    ...
    return `https://embed.windy.com/embed2.html?lat=20&lon=0&zoom=2&level=surface&overlay=${overlay}&product=gfs&menu=&message=&marker=off&calendar=now&pressure=&type=map&location=coordinates&detail=on&metricWind=mph&metricTemp=degF&radarRange=-1&autoplay=1&numDirection=1&windLabels=1`;
  };
    ...
    <Box sx={{ height: '100%', width: '100%', display: 'flex', overflow: 'hidden' }}>
      ...
      <Box sx={{
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              <FormControlLabel
                ...
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">[FIRE] Active Fire Markers ({fireData.length})</Typography>
                </Box>}
              />
              <FormControlLabel
                control={<Switch checked={true} disabled sx={{ '& .MuiSwitch-thumb': { backgroundColor: 'white' } }} />}
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">[HELICOPTER] Helicopter Landing Zones</Typography>
                </Box>}
              />
              <FormControlLabel
                control={<Switch checked={true} disabled sx={{ '& .MuiSwitch-thumb': { backgroundColor: 'white' } }} />}
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">[DROPLET] Water Sources</Typography>
                </Box>}
              />
              <FormControlLabel
                control={<Switch checked={true} disabled sx={{ '& .MuiSwitch-thumb': { backgroundColor: 'white' } }} />}
              ...
              <FormControlLabel
                ...
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">[SATELLITE_ANTENNA] Communication Towers</Typography>
                </Box>}
              />
              <FormControlLabel
                control={<Switch checked={true} disabled sx={{ '& .MuiSwitch-thumb': { backgroundColor: 'white' } }} />}
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Home sx={{ fontSize: 16 }} />
                  <Typography variant="body2">[HOUSE] Wildland Urban Interface</Typography>
                </Box>}
              />
              <FormControlLabel
                control={<Switch checked={true} disabled sx={{ '& .MuiSwitch-thumb': { backgroundColor: 'white' } }} />}
              ...
              <FormControlLabel
                ...
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">[HERB] Fuel Load Analysis</Typography>
                </Box>}
              />
              <FormControlLabel
                control={<Switch checked={true} disabled sx={{ '& .MuiSwitch-thumb': { backgroundColor: 'white' } }} />}
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">[THERMOMETER] Weather Stations</Typography>
                </Box>}
              />
              <FormControlLabel
                control={<Switch checked={true} disabled sx={{ '& .MuiSwitch-thumb': { backgroundColor: 'white' } }} />}
                label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">[FIRE] Fire Hazard Severity Zones</Typography>
                </Box>}
              />
            </Box>
          </Card>
        ...
        <Box sx={{
          ...
          <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
            [SATELLITE] COMPREHENSIVE FIRE INTELLIGENCE
          </Typography>
          <Typography variant="caption" sx={{ display: 'block' }}>
