...
const LiveFireDataMap: React.FC<LiveFireDataMapProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log('[MAP] Live Fire Data Map initialized');
    } catch (err) {
  ...
  const addLiveFireMarkers = useCallback((map: any, fireData: any[]) => {
    ...
    console.log(`[FIRE] Adding ${fireData.length} live fire markers to map`);
        ...
        .bindPopup(`
          <div style="min-width: 300px; font-family: Arial, sans-serif;">
            <h3 style="margin: 0 0 10px 0; color: #FF4500; display: flex; align-items: center;">
              [SATELLITE] LIVE SATELLITE FIRE DETECTION
            </h3>
            ...
            <div style="margin-bottom: 10px;">
              ...
              ${fire.latitude.toFixed(4)}degN, ${Math.abs(fire.longitude).toFixed(4)}degW
            </div>
            <div style="margin-bottom: 10px;">
  ...
  const addOperationalOverlays = useCallback((map: any) => {
    ...
    console.log('[HELICOPTER] Adding operational overlays to map');
    ...
    operationalData.forEach((overlay) => {
      ...
      switch (overlay.type) {
        ...
        case 'emergency_service':
          iconHtml = overlay.name.includes('Fire') ? '🚒' :
                    overlay.name.includes('Police') ? '🚔' : '🚑';
          color = '#F44336';
          break;
          ...
          <div style="min-width: 250px; font-family: Arial, sans-serif;">
            ...
            <div style="background: ${overlay.status === 'threatened' ? 'rgba(255, 152, 0, 0.1)' : 'rgba(76, 175, 80, 0.1)'};
              ...
              <strong style="color: ${overlay.status === 'threatened' ? '#FF9800' : '#4CAF50'};">
                ${overlay.status === 'threatened' ? '[WARNING] Asset at Risk' : '[CHECK] Operational Status'}
              </strong><br>
        ...
        <Card sx={{
          ...
          <Box sx={{ mt: 2, pt: 2, borderTop: '1px solid rgba(255,255,255,0.3)' }}>
            ...
            <Typography variant="caption" sx={{ display: 'block', mt: 0.5 }}>
              [HELICOPTER] {operationalData.filter(o => o.type === 'helicopter_landing').length} Helicopter Zones
            </Typography>
            <Typography variant="caption" sx={{ display: 'block' }}>
              [DROPLET] {operationalData.filter(o => o.type === 'water_source').length} Water Sources
            </Typography>
          </Box>
        </Card>
      </Collapse>
